...
local S = {
    ...
    Connections = {},
    ChatConnections = {},
    ESPPool = {},      -- [Player] = { BoxOutline, BoxFill, TracerLine, NameTag, HealthText, DistText, HealthBarOutline, HealthBarFill, Skeleton = {lines} }
    ...
    MacroText = "WeAreSkidding On Top!",
    ...
    UserIDGrabKey = Enum.KeyCode.K,

    currentOptionsModule = ""
}

-- Statistics row wrappers
...
destroyESP = function(p)
    ...
    if pool then
        ...
        if pool.skeleton then
            for _, line in ipairs(pool.skeleton) do
        ...
        for _, log in ipairs(history) do
            ...
            if existingLog then
                ...
                for idx, item in ipairs(consoleLogs) do
                    if item == existingLog then table.remove(consoleLogs, idx); break end
                end
                ...
                if #consoleLogs > 500 then
                    ...
                    if removed then consoleLogsMap[removed.messageType.Value .. "_" .. removed.message] = nil end
                end
            end
        end
    end)
...
local function connectChatLogger()
    ...
    pcall(function()
        if chatService.ChatVersion == Enum.ChatVersion.TextChatService then useModern = true end
    end)
...
local function createOverhead(p)
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then
        local connection = hum.HealthChanged:Connect(updateText)
...
local function refreshOverheads()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and p.Character then createOverhead(p) end
    end
end

local function saveFavorites()
    pcall(function()
        if writefile then writefile("utility_hub_favorites.json", HttpService:JSONEncode(S.FavoriteMaps)) end
    end)
end
...
local function loadConfig()
    ...
    -- Reset states that cause issues if left on
    ...
    S.WallRun = false; S.FlyBypass = false
end
loadConfig()
setupAutoRejoin()
setupAutoReinject()
...
-- Font Protection Logic: Prevents the game from changing our UI fonts
...
local hudWatermark, hudCoords, hudArrayListFrame = nil, nil, nil
...
updateHUDArrayList = function()
    ...
    for modName, item in pairs(moduleButtons) do
        if item.IsActive and item.IsActive() then table.insert(activeMods, modName) end
    end
...
task.spawn(function()
    while true do task.wait(0.2); pcall(updateHUDArrayList) end
end)
...
local function findWindowFrame(obj)
    local curr = obj
    while curr do
        if curr:IsA("Frame") and curr:GetAttribute("BaseWidth") then return curr end
        curr = curr.Parent
    end
    return nil
end
...
local function addDropdownOption(parent, name, optionsList, defaultValIndex, callback)
    ...
    local function toggleDropdown()
        ...
        if winFrame then
            ...
            if baseW > 0 then scale = winFrame.Size.X.Offset / baseW end
        end
        if open then
...
local function createFloatingWindow(title, width, height, defaultX, defaultY)
    ...
    win:GetPropertyChangedSignal("Size"):Connect(function()
        if collapsed or isScaling then return end
        isScaling = true
        ...
        if baseWidth > 0 then
            ...
            local pV = lL and lL.Padding.Offset or (4 * scale)
            local uP = content:FindFirstChildOfClass("UIPadding")
            local pT = uP and uP.PaddingTop.Offset or (6 * scale)
...
local function registerModule(catName, name, defaultX, defaultY, isToggle, defaultState, callback, populateOptionsFunc, useSeparateWindow, winWidth, winHeight)
    ...
    local function updateBg()
        ...
        if useSeparateWindow and floatingWin then isOpened = floatingWin.Visible
        elseif drawer then isOpened = drawer.Visible end
        if isOpened then
            ...
            if open then
                ...
                if optionsFrame then
                    for _, child in ipairs(optionsFrame:GetChildren()) do
                        if child:IsA("Frame") then targetHeight = targetHeight + child.Size.Y.Offset + 3 end
                    end
                end
                ...
                tween.Completed:Connect(function() drawer.AutomaticSize = Enum.AutomaticSize.Y; isTweening = false end)
                tween:Play()
                ...
                tween.Completed:Connect(function() drawer.Visible = false; updateBg(); isTweening = false end)
                tween:Play()
...
local Win = {
    ...
    SetAutoReinject = function(self, val, callback) end,
    SetOnClose = function(self, callback) end
}
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for modName, item in pairs(moduleButtons) do
        if text == "" or modName:lower():find(text) then item.Container.Visible = true
        else item.Container.Visible = false end
    end
end)
...
local function selectTab(tabName)
    ...
    for _, win in ipairs(floatingWindows) do
        if isModules then win.Visible = win:GetAttribute("UserOpen") == true
...
local function createPanel(title, width, height)
    ...
    closeBtn.Text = "X"
    closeBtn.Parent = header
    closeBtn.MouseButton1Click:Connect(function() win.Visible = false; selectTab("Modules") end)
    local content = Instance.new("ScrollingFrame")
    content.Name = "content"
    content.Size = UDim2.new(1, 0, 1, -22)
    content.Position = UDim2.new(0, 0, 0, 22)
...
addDropdownOption(settingsContent, "Interface Theme Color", {"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, table.find({"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, S.ThemeColor) or 1, function(_, opt)
    applyThemeColor(opt); saveConfig()
end)
addKeybindOption(settingsContent, "Menu Toggle Keybind", S.UIToggleKey or Enum.KeyCode.RightControl, function(k)
    S.UIToggleKey = k; saveConfig(); notify("UI Toggle Keybind set to: " .. k.Name, Color3.fromRGB(50, 195, 75))
end)
...
local capsList = {
    ...
    {"queue_on_teleport", queue_on_teleport or queueteleport}
}
for _, cap in ipairs(capsList) do
    totalFuncs = totalFuncs + 1
    if cap[2] then supportedFuncs = supportedFuncs + 1 end
end
...
registerModule("Combat", "God Mode", 20, 50, true, S.GodMode, function(v)
    S.GodMode = v
    if v then if LP.Character then applyGodMode(LP.Character) end else disableGodMode() end
    saveConfig()
end)

registerModule("Combat", "Kill Aura", 20, 50, true, S.KillAura, function(v)
    S.KillAura = v; saveConfig()
end, function(drawer)
    addSliderOption(drawer, "Range (studs)", 5, 50, S.KillAuraRange, function(v) S.KillAuraRange = v; saveConfig() end)
end, false)
...
registerModule("Combat", "Touch Aura", 20, 50, true, S.TouchAura, function(v) S.TouchAura = v; saveConfig() end, function(drawer)
    addSliderOption(drawer, "Range (studs)", 5, 30, 15, function(v) S.KillAuraRange = v; saveConfig() end)
end, false)
...
registerModule("Combat", "Aimbot", 20, 50, true, S.AimbotActive, function(v)
    S.AimbotActive = v; notify("Aimbot " .. (v and "Enabled" or "Disabled"), Color3.fromRGB(50, 195, 75)); saveConfig()
end, function(drawer)
...
registerModule("Combat", "Aimlock", 20, 50, true, S.AimlockActive, function(v)
    S.AimlockActive = v; notify("Aimlock " .. (v and "Enabled" or "Disabled"), Color3.fromRGB(50, 195, 75)); saveConfig()
end, function(drawer)
...
registerModule("Combat", "Triggerbot", 20, 50, true, S.TriggerbotActive, function(v)
    S.TriggerbotActive = v; notify("Triggerbot " .. (v and "Enabled" or "Disabled"), Color3.fromRGB(50, 195, 75)); saveConfig()
end, function(drawer)
...
registerModule("Combat", "Fling Player", 20, 50, true, S.FlingActive, function(v)
    ...
    if v then S.FlingAllActive = false; local mod = moduleButtons["Fling All"]; if mod then mod.SetActive(false) end
        ...
        task.spawn(function()
            ...
            if hrp then
                ...
                if S.LastSafePosition then hrp.CFrame = S.LastSafePosition end
                task.wait(0.05)
                if hrp:IsDescendantOf(game) then hrp.AssemblyLinearVelocity = Vector3.zero; hrp.AssemblyAngularVelocity = Vector3.zero end
            end
        end)
    end
    saveConfig()
    ...
    addTextboxOption(drawer, "Fling Target Player", "Username", function(txt)
        ...
        local found = nil
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP and (p.Name:lower():find(txt:lower()) or p.DisplayName:lower():find(txt:lower())) then found = p; break end
        end
...
registerModule("Player", "Reset Character", 160, 50, false, false, function()
    local hum = getHum(); if hum then hum.Health = 0; notify("Character reset!", Color3.fromRGB(218, 38, 38)) end
end)
...
registerModule("Player", "Give BTools", 160, 50, false, false, function()
    for i = 1, 4 do local t = Instance.new("HopperBin"); t.BinType = i; t.Parent = LP.Backpack end
...
local function applySpectate()
    ...
    if #list == 0 then notify("No targets in server to spectate", Color3.fromRGB(218, 38, 38)); return end
...
registerModule("Player", "Spectate & Freecam", 160, 50, true, false, function(v) if not v then resetCameraToSelf() end end, function(drawer)
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP and p.Character then
                ...
                if root then
                    ...
                    if dist < shortestDist then shortestDist = dist; nearest = p end
                end
            end
        end
        if nearest then
    ...
    local scroll = Instance.new("ScrollingFrame")
    scroll.Size = UDim2.new(1, -8, 1, -16); scroll.Position = UDim2.new(0, 4, 0, 16); scroll.BackgroundTransparency = 1; scroll.BorderSizePixel = 0; scroll.ScrollBarThickness = 1; scroll.CanvasSize = UDim2.new(0, 0, 0, 0); scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; scroll.Parent = listContainer
    local layout = Instance.new("UIListLayout")
    layout.Padding = UDim.new(0, 2); layout.Parent = scroll
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP then
                ...
                if filter == "" or formatted:lower():find(filter) then
                    ...
                    card.Size = UDim2.new(1, -2, 0, 16); card.BackgroundColor3 = Color3.fromRGB(25, 25, 25); card.BorderSizePixel = 0; card.Parent = scroll
                    local nameL = Instance.new("TextLabel")
                    nameL.Size = UDim2.new(0.5, 0, 1, 0); nameL.Position = UDim2.new(0, 2, 0, 0); nameL.BackgroundTransparency = 1; nameL.Font = Enum.Font.GothamMedium; nameL.TextSize = 7; nameL.TextColor3 = Color3.fromRGB(220, 220, 220); nameL.TextXAlignment = Enum.TextXAlignment.Left; nameL.Text = p.DisplayName; nameL.Parent = card
                    local tp = Instance.new("TextButton")
                    tp.Size = UDim2.new(0, 20, 0, 12); tp.Position = UDim2.new(1, -44, 0.5, -6); tp.BackgroundColor3 = Color3.fromRGB(46, 204, 113); tp.BorderSizePixel = 0; tp.Font = Enum.Font.GothamBold; tp.TextSize = 6; tp.TextColor3 = Color3.fromRGB(255, 255, 255); tp.Text = "TP"; tp.Parent = card
                    tp.MouseButton1Click:Connect(function()
                        local targetHRP = p.Character and p.Character:FindFirstChild("HumanoidRootPart")
                        if targetHRP and teleportToHRP(targetHRP) then notify("Teleported to " .. p.DisplayName, Color3.fromRGB(50, 195, 75))
                        else notify("Target not loaded", Color3.fromRGB(218, 38, 38)) end
                    end)
                    local view = Instance.new("TextButton")
                    view.Size = UDim2.new(0, 22, 0, 12); view.Position = UDim2.new(1, -22, 0.5, -6)
                    local isViewing = (currentSpectateTarget == p)
                    view.BackgroundColor3 = isViewing and Color3.fromRGB(218, 38, 38) or Color3.fromRGB(40, 40, 40); view.BorderSizePixel = 0; view.Font = Enum.Font.GothamBold; view.TextSize = 6; view.TextColor3 = Color3.fromRGB(255, 255, 255); view.Text = isViewing and "UNVIEW" or "VIEW"; view.Parent = card
                    view.MouseButton1Click:Connect(function()
                        if currentSpectateTarget == p then spectatePlayer(nil) else spectatePlayer(p) end
                        renderPlayers()
                    end)
                end
            end
    ...
    local removedCon = Players.PlayerRemoving:Connect(function(p) if currentSpectateTarget == p then spectatePlayer(nil) end; renderPlayers() end)
    table.insert(S.Connections, addedCon); table.insert(S.Connections, removedCon)
    renderPlayers()
end, true, 200, 280)

registerModule("Movement", "Speed Modification", 300, 50, true, S.ForceWalkSpeed, function(v)
    S.ForceWalkSpeed = v; local hum = getHum(); if hum then hum.WalkSpeed = v and S.WalkSpeed or gameDefaultSpeed end; saveConfig()
end, function(drawer)
...
registerModule("Movement", "Sprint Speed Boost", 300, 50, true, S.SprintEnabled, function(v)
    S.SprintEnabled = v; if not v then local hum = getHum(); if hum then hum.WalkSpeed = (S.ForceWalkSpeed and S.WalkSpeed) or gameDefaultSpeed end end; saveConfig()
end, function(drawer) addSliderOption(drawer, "Sprint Speed factor", 20, 150, S.SprintSpeed, function(v) S.SprintSpeed = v; saveConfig() end) end, false)

registerModule("Movement", "Jump Hack Strength", 300, 50, true, S.ForceJumpPower, function(v)
    S.ForceJumpPower = v; local hum = getHum(); if hum then if v then hum.UseJumpPower = true; hum.JumpPower = S.JumpPower else hum.UseJumpPower = gameDefaultUseJumpPower; hum.JumpPower = gameDefaultJumpPower end end; saveConfig()
end, function(drawer)
...
registerModule("Movement", "Climb", 300, 50, true, S.Climb, function(v)
    S.Climb = v
    if not v then local hum = getHum(); if hum then hum.PlatformStand = false end end
    saveConfig()
end, function(drawer)
    addSliderOption(drawer, "Climb Speed", 5, 50, S.ClimbSpeed, function(v) S.ClimbSpeed = v; saveConfig() end)
end, false)

registerModule("Movement", "Wall Run", 300, 50, true, S.WallRun, function(v)
    S.WallRun = v; saveConfig()
end)

registerModule("Movement", "Fly Mode", 300, 50, true, S.Fly, function(v)
    S.Fly = v; if v then flyOn() else flyOff() end; saveConfig()
end, function(drawer) addSliderOption(drawer, "Fly Speed factor", 10, 300, S.FlySpeed, function(v) S.FlySpeed = v; saveConfig() end) end, false)

registerModule("Movement", "Fly Bypass", 300, 50, true, S.FlyBypass, function(v)
    S.FlyBypass = v
    if v then S.Fly = false; flyOff(); local mod = moduleButtons["Fly Mode"]; if mod then mod.SetActive(false) end end
    saveConfig()
end, function(drawer) addSliderOption(drawer, "Bypass Speed factor", 10, 150, S.FlySpeed, function(v) S.FlySpeed = v; saveConfig() end) end, false)

registerModule("Movement", "Infinite Jump", 300, 50, true, S.InfJump, function(v) S.InfJump = v; saveConfig() end)
registerModule("Movement", "Auto Bunnyhop", 300, 50, true, S.BHop, function(v) S.BHop = v; saveConfig() end)

registerModule("Movement", "Air Walk Platform", 300, 50, true, S.AirWalk, function(v)
    S.AirWalk = v; saveConfig()
end)
...
registerModule("Movement", "Ghost State Mode", 300, 50, true, S.GhostMode, function(v)
    S.GhostMode = v; if v then enableGhostMode() else disableGhostMode() end; saveConfig()
end, function(drawer) addToggleOption(drawer, "Teleport to Ghost End", S.GhostTeleportToEnd, function(v) S.GhostTeleportToEnd = v; saveConfig() end) end, false)
...
registerModule("Render", "Chams", 440, 50, true, S.Chams, function(v)
    ...
    if not v then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP and p.Character then
                local hl = p.Character:FindFirstChild("VoidChams")
...
registerModule("World", "Instant Prompts", 580, 50, true, S.InstantPrompts, function(v)
    S.InstantPrompts = v; if v then for _, p in ipairs(Workspace:GetDescendants()) do if p:IsA("ProximityPrompt") then p.HoldDuration = 0 end end end; saveConfig()
end)
...
registerModule("World", "Save Current Location", 580, 50, false, false, function()
    local hrp = getHRP(); if hrp then S.SavedWaypointCF = hrp.CFrame; notify("Current location locked!", Color3.fromRGB(50, 195, 75)) else notify("HumanoidRootPart not found", Color3.fromRGB(218, 38, 38)) end
end)
registerModule("World", "Warp to Saved Location", 580, 50, false, false, function()
    if S.SavedWaypointCF then local hrp = getHRP(); if hrp then hrp.CFrame = S.SavedWaypointCF; notify("Returned to saved waypoint!", Color3.fromRGB(50, 195, 75)) else notify("HumanoidRootPart not found", Color3.fromRGB(218, 38, 38)) end
    else notify("No waypoint saved yet", Color3.fromRGB(218, 38, 38)) end
end)
registerModule("World", "Destroy Killbricks", 580, 50, false, false, function()
    ...
    for _, v in ipairs(Workspace:GetDescendants()) do
        if v:IsA("TouchTransmitter") and v.Parent and not v.Parent:IsDescendantOf(LP.Character) then
            ...
            if parentName:match("kill") or parentName:match("lava") or v.Parent.BrickColor.Name == "Bright red" then v:Destroy(); count = count + 1 end
        end
    end
...
registerModule("World", "Destroy Seats", 580, 50, false, false, function()
    local count = 0
    for _, v in ipairs(Workspace:GetDescendants()) do
        if (v:IsA("Seat") or v:IsA("VehicleSeat")) and not v:IsDescendantOf(LP.Character) then pcall(function() v:Destroy(); count = count + 1 end) end
    end
...
local function rebuildFriends(scroll)
    ...
    task.spawn(function()
        ...
        if not ok or not onlineFriends then
            local empty = Instance.new("TextLabel"); empty.Text = "Failed to query friends."; empty.Font = Enum.Font.Gotham; empty.TextSize = math.clamp(math.round(8 * scale), 7, 24); empty.Size = UDim2.new(1, 0, 0, 14 * scale); empty.TextColor3 = Color3.fromRGB(120, 120, 120); empty.BackgroundTransparency = 1; empty.Parent = scroll
            return
        end
        for _, item in ipairs(onlineFriends) do
            ...
            if item.LocationType == 1 or item.LocationType == 4 or item.LocationType == 5 or (item.GameId and item.GameId ~= "") then
                if item.PlaceId and item.PlaceId > 0 then isInGame = true; statusText = "🎮 Play: " .. (item.LastLocation or "In-game") end
            elseif item.LocationType == 3 then statusText = "🔧 Studio"; statusColor = Color3.fromRGB(218, 170, 42) end
            ...
            if isInGame then
                ...
                join.MouseButton1Click:Connect(function()
                    if item.GameId and item.GameId ~= "" then notify("Connecting to friend...", Color3.fromRGB(50, 195, 75)); pcall(function() TeleportService:TeleportToPlaceInstance(item.PlaceId, item.GameId, LP) end)
                    else notify("Warping to friend...", Color3.fromRGB(50, 195, 75)); pcall(function() TeleportService:Teleport(item.PlaceId, LP) end) end
                end)
            end
        end
    end)
...
registerModule("Misc", "Online Friends", 720, 50, false, false, nil, function(drawer)
    ...
    local refreshBtn = Instance.new("TextButton"); refreshBtn.Size = UDim2.new(1, -8, 0, 14); refreshBtn.Position = UDim2.new(0, 4, 0, 0); refreshBtn.BackgroundColor3 = currentThemeColor; refreshBtn.BorderSizePixel = 0; refreshBtn.Font = Enum.Font.GothamBold; refreshBtn.TextSize = 8; refreshBtn.TextColor3 = Color3.fromRGB(255, 255, 255); refreshBtn.Text = "Refresh Online Friends"; refreshBtn.Parent = frame
...
registerModule("Misc", "Chat Logger", 720, 50, false, false, nil, function(drawer)
    ...
    addTextboxOption(drawer, "Filter chat logs text", "Filter text", function(txt)
        ...
        for _, log in ipairs(S.ChatHistory) do
            local matches = true
            if filter ~= "" then matches = log.Speaker:lower():find(filter:lower()) or log.Message:lower():find(filter:lower()) end
            if matches then chatFeed:AddEntry(string.format("[%s] [%s]: %s", log.Timestamp, log.Speaker, log.Message), log.Color) end
        end
    end)
    addButtonOption(drawer, "Copy Entire Chat Logs", function()
        local text = ""
        for _, log in ipairs(S.ChatHistory) do text = text .. string.format("[%s] [%s]: %s\n", log.Timestamp, log.Speaker, log.Message) end
...
registerModule("Misc", "Console Log Viewer", 720, 50, false, false, nil, function(drawer)
    ...
    local showI, showW, showE = true, true, true
    local function rebuildConsole()
        ...
        for _, log in ipairs(consoleLogs) do
            ...
            if mType == Enum.MessageType.MessageOutput or mType == Enum.MessageType.MessageInfo then
                if mType == Enum.MessageType.MessageInfo then col = Color3.fromRGB(80, 180, 240); prefix = "[INFO] " end
                show = showI
...
function enableGhostMode()
    ...
    S.Fly = true; flyOn(); S.NoClip = true
...
function flyOn()
    ...
    bv.Name = "VoidFlyBV"; bv.MaxForce = Vector3.new(9e9, 9e9, 9e9); bv.Velocity = Vector3.zero; bv.Parent = hrp
    local bg = Instance.new("BodyGyro")
    bg.Name = "VoidFlyBG"; bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bg.D = 100; bg.Parent = hrp
end
...
table.insert(S.Connections, RunService.RenderStepped:Connect(function()
    ...
    if S.AimbotHoldMode == "Keyboard" then
        if S.AimbotHoldKey and S.AimbotHoldKey ~= Enum.KeyCode.Unknown then aimbotPressed = UserInputService:IsKeyDown(S.AimbotHoldKey) end
    else
    ...
    if S.AimlockActive then
        ...
        if targetPart then
            local goalCF = CFrame.new(Camera.CFrame.Position, targetPart.Position)
            local sm = math.max(S.AimlockSmooth, 1)
            Camera.CFrame = Camera.CFrame:Lerp(goalCF, 1 / sm)
        end
    end
            ...
            if target then
                ...
                while current and current ~= game do
                    if current:IsA("Model") then
                        ...
                        if h then char = current; hum = h; break end
                    end
                            ...
                            if not lastTriggerFire or (now - lastTriggerFire) >= (S.TriggerbotDelay or 0.05) then
                                ...
                                pcall(function()
                                    if mouse1press and mouse1release then
                                        task.spawn(function() mouse1press(); task.wait(0.01); mouse1release() end)
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LP and p.Character then
                ...
                if not char:FindFirstChild("VoidChams") then
                    ...
                    hl.Name = "VoidChams"
                    hl.FillColor = p.Team and p.Team.TeamColor.Color or Color3.fromRGB(218, 38, 38)
    ...
    -- ViewModel FOV Update
    if S.ViewModelFOV ~= 70 then
        local hum = getHum()
        if hum and hum.Health > 0 then
            -- Detect First Person
            local hrp = getHRP()
            if hrp and (Camera.Focus.Position - Camera.CFrame.Position).Magnitude < 1 then
                Camera.FieldOfView = S.ViewModelFOV
            else
    ...
    if espEnabled then
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            if valid then
                ...
                if S.ESPDistanceColor then
                    local pct = math.clamp(dist / 500, 0, 1)
                    espDrawCol = Color3.fromRGB(255 * pct, 255 * (1 - pct), 0) -- Red close, Green far
                end
                ...
                if not S.ESPPool[p] then
                    ...
                    for i=1, 15 do
                        table.insert(S.ESPPool[p].skeleton, Drawing.new("Line"))
                    end
                end
                local pool = S.ESPPool[p]
                ...
                if box and sp.Z > 0 then
                    ...
                    local vp = Camera.ViewportSize
                    local originY = vp.Y
                    if S.TracerOrigin == "Center" then originY = vp.Y / 2
                    elseif S.TracerOrigin == "Top" then originY = 0 end
                    ...
                    local healthBarOutline = pool.healthBarOutline
                    healthBarOutline.Visible = S.ESPHealth
                    healthBarOutline.Position = Vector2.new(topLeft.X - 5, topLeft.Y)
                    healthBarOutline.Size = Vector2.new(2, height)
                    healthBarOutline.Color = Color3.new(0, 0, 0)
                    healthBarOutline.Thickness = 1
                    healthBarOutline.Filled = true

                    local healthBarFill = pool.healthBarFill
                    healthBarFill.Visible = S.ESPHealth
                    healthBarFill.Position = Vector2.new(topLeft.X - 4, topLeft.Y + 1)
                    healthBarFill.Size = Vector2.new(1, (height - 2) * hpPct)
                    healthBarFill.Color = Color3.fromRGB(255 * (1 - hpPct), 255 * hpPct, 0)
                    ...
                    nameTag.Position = Vector2.new(topLeft.X + width / 2, topLeft.Y - 16)
                        ...
                        for i, bone in ipairs(useBones) do
                            ...
                            if line then
                                ...
                                if part1 and part2 then
                                    local sp1, on1 = Camera:WorldToViewportPoint(part1.Position)
                                    local sp2, on2 = Camera:WorldToViewportPoint(part2.Position)
                                    if on1 and on2 then
                                        line.Visible = true
                    ...
                    pool.healthBarOutline.Visible = false
                    pool.healthBarFill.Visible = false
                    for _, line in ipairs(pool.skeleton) do line.Visible = false end
                end
            else
                destroyESP(p)
...
local function getNextFlingAllTarget(currentTarget)
    ...
    if currentTarget then
        for idx, p in ipairs(candidates) do
            if p == currentTarget then currentIndex = idx; break end
        end
    end
...
table.insert(S.Connections, RunService.Heartbeat:Connect(function(dt)
    ...
    -- Instant Respawn
    if S.InstantRespawn and myHum and myHum.Health <= 0 then
        task.wait()
        LP:LoadCharacter()
    end
    ...
    if S.FlyBypass and myHum and myHRP then
        ...
        if not myHRP:FindFirstChild("VoidBypassFly") then
            local bv = Instance.new("BodyVelocity")
            bv.Name = "VoidBypassFly"
        ...
        if S.Climb and myChar and myHRP and myHum then
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then
                ...
                if result and result.Instance then
                    myHum.PlatformStand = true
                    myHRP.CFrame = myHRP.CFrame + Vector3.new(0, S.ClimbSpeed * dt, 0)
                    myHRP.AssemblyLinearVelocity = Vector3.zero
                else
        ...
        if S.WallRun and myChar and myHRP and myHum then
            if UserInputService:IsKeyDown(Enum.KeyCode.W) and myHum.FloorMaterial == Enum.Material.Air then
                ...
                if rightRay or leftRay then
                    local upVel = Vector3.new(0, 10, 0)
                    local fwdVel = myHRP.CFrame.LookVector * 20
    ...
    pcall(function()
        if myHum then
            ...
            if isSprinting then myHum.WalkSpeed = S.SprintSpeed
            elseif S.ForceWalkSpeed then myHum.WalkSpeed = S.WalkSpeed end
            if S.ForceJumpPower then myHum.UseJumpPower = true; myHum.JumpPower = S.JumpPower end
        end
    end)
    
    -- Proper Bunny Hop Implementation
    pcall(function()
        if S.BHop and myHRP and myHum then
            ...
            if moveDir.Magnitude > 0.1 then
                ...
                if myHum.FloorMaterial ~= Enum.Material.Air then
                    ...
                    if speed > 0.1 then
                        local newHorizontal = (horizontalVel + moveDir * 2).Unit * speed
                        myHRP.AssemblyLinearVelocity = Vector3.new(newHorizontal.X, currentVel.Y, newHorizontal.Z)
                    else
                        myHRP.AssemblyLinearVelocity = Vector3.new((moveDir * 16).X, currentVel.Y, (moveDir * 16).Z)
                    end
                end
            end
        end
            ...
            if myHum and myHRP then
                if myHum.FloorMaterial == Enum.Material.Air then
                    if not S.AirWalkPlat then
                        ...
                        plat.Size = Vector3.new(6, 1, 6) -- Thicker so it catches properly
                        plat.Anchored = true
        ...
        if S.WaterWalk and myHRP and myChar then
            ...
            if raycastResult and raycastResult.Material == Enum.Material.Water then
                if not S.WaterPlat then
                    local plat = Instance.new("Part")
                    plat.Name = "VoidWaterPlat"; plat.Size = Vector3.new(100, 1, 100); plat.Anchored = true; plat.Transparency = 1; plat.CanCollide = true; plat.Parent = Workspace
                    S.WaterPlat = plat
                end
...
table.insert(S.Connections, UserInputService.InputBegan:Connect(function(inp, gpe)
    ...
    if S.PanicKey and inp.KeyCode == S.PanicKey then
        ...
        updateHUDArrayList()
        notify("PANIC! All modules disabled.", Color3.fromRGB(218, 38, 38))
        return
    end
    ...
    if S.UserIDGrabKey and inp.KeyCode == S.UserIDGrabKey then
        ...
        local char = mouseHit and mouseHit.Parent
        local p = char and Players:GetPlayerFromCharacter(char)
        if p then
            if setclipboard then setclipboard(tostring(p.UserId)) end
            notify("Copied UserID: " .. p.UserId .. " (" .. p.DisplayName .. ")", Color3.fromRGB(50, 195, 75))
        else
            notify("No player found under mouse.", Color3.fromRGB(218, 38, 38))
        end
        return
    end
    ...
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
        if S.ClickDelete and UserInputService:IsKeyDown(Enum.KeyCode.LeftAlt) then
            local target = Mouse.Target
            if target and not target.Parent:FindFirstChildOfClass("Humanoid") then target:Destroy() end
    ...
    if S.MacroKey and S.MacroKey ~= Enum.KeyCode.Unknown and k == S.MacroKey and S.MacroText and S.MacroText ~= "" then
        pcall(function()
            ...
            if chatService and chatService.ChatVersion == Enum.ChatVersion.TextChatService then
                ...
                if textChannel then textChannel:SendAsync(S.MacroText) end
            else
                ...
                if sayMsg then sayMsg:FireServer(S.MacroText, "All") end
            end
        end)
    end
    ...
    elseif S.FlyKey and S.FlyKey ~= Enum.KeyCode.Unknown and k == S.FlyKey then
        S.Fly = not S.Fly; if S.Fly then flyOn() else flyOff() end; notify("Fly Mode " .. (S.Fly and "ON" or "OFF"), Color3.fromRGB(218, 170, 42))
        local mod = moduleButtons["Fly Mode"]; if mod then mod.SetActive(S.Fly) end
    elseif S.NoClipKey and S.NoClipKey ~= Enum.KeyCode.Unknown and k == S.NoClipKey then
        S.NoClip = not S.NoClip; notify("NoClip " .. (S.NoClip and "ON" or "OFF"), Color3.fromRGB(218, 170, 42))
        local mod = moduleButtons["NoClip Passes"]; if mod then mod.SetActive(S.NoClip) end
    elseif S.BHopKey and S.BHopKey ~= Enum.KeyCode.Unknown and k == S.BHopKey then
        S.BHop = not S.BHop; notify("Bunnyhop " .. (S.BHop and "ON" or "OFF"), Color3.fromRGB(218, 170, 42))
        local mod = moduleButtons["Auto Bunnyhop"]; if mod then mod.SetActive(S.BHop) end
    elseif S.InfJumpKey and S.InfJumpKey ~= Enum.KeyCode.Unknown and k == S.InfJumpKey then
        S.InfJump = not S.InfJump; notify("Infinite Jump " .. (S.InfJump and "ON" or "OFF"), Color3.fromRGB(218, 170, 42))
        local mod = moduleButtons["Infinite Jump"]; if mod then mod.SetActive(S.InfJump) end
    elseif S.JumpStrengthKey and S.JumpStrengthKey ~= Enum.KeyCode.Unknown and k == S.JumpStrengthKey then
        ...
        local hum = getHum()
        if hum then
            if S.ForceJumpPower then hum.UseJumpPower = true; hum.JumpPower = S.JumpPower
            else hum.UseJumpPower = gameDefaultUseJumpPower; hum.JumpPower = gameDefaultJumpPower end
        end
        notify("Jump Strength " .. (S.ForceJumpPower and "ON" or "OFF"), Color3.fromRGB(218, 170, 42))
        local mod = moduleButtons["Jump Hack Strength"]; if mod then mod.SetActive(S.ForceJumpPower) end
        saveConfig()
    ...
    elseif S.BlinkKey and S.BlinkKey ~= Enum.KeyCode.Unknown and k == S.BlinkKey then
        ...
        if hrp and hum then
            local dir
            if S.BlinkDirection == "Camera Look" then dir = Camera.CFrame.LookVector
            else dir = hum.MoveDirection.Magnitude > 0 and hum.MoveDirection or hrp.CFrame.LookVector end
