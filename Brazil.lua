...
-- Pre-declare functions to fix scoping issues and memory leaks
local destroyESP, restoreHitbox, cleanupAll
local toggleMapXray, toggleClearVision, toggleGraphicsReducer
local applyTallAnimations, revertTallAnimations
local applyGodMode, disableGodMode
local toggleFloat, toggleWaterWalk
local notify, showToast, updateHUDArrayList
local currentThemeColor = Color3.fromRGB(141, 47, 196) -- Default Purple
local applyHitbox, updateHitboxes
local getAimbotTarget
local applyCustomIdle
local updateLocalNametag
local processUISpoofText
...
local S = {
    ...
    ESPTeamCheck = false, ESPTransparency = 0.8, ESPDistanceColor = false, SkeletonESP = false, Chams = false,
    OverheadInfo = false,
    AimbotActive = false, AimbotTeamCheck = false, AimbotFOV = 120, AimbotSmooth = 5, AimbotPart = "Head",
    ...
    AntiSit = false, GraphicsReducer = false, AutoRejoin = false, FreecamSpeed = 40, TracerOrigin = "Bottom",
    ...
    TriggerbotActive = false, TriggerbotTeamCheck = true, TriggerbotDelay = 0.05,
...
local rowHomeChatFeed = { AddEntry = function(self, text, color) if activeChatFeed then activeChatFeed:AddEntry(text, color) end end }
...
local rowConsoleFeed = {
    ...
    AddEntry = function(self, text, color) if activeConsoleFeed then activeConsoleFeed:AddEntry(text, color) end end
}

-- ──────────────────────────────────────────────────────────────
--  CLEANUP ROUTINE
-- ──────────────────────────────────────────────────────────────
destroyESP = function(p)
...
cleanupAll = function()
    pcall(function()
        ...
        if pg then local oldPg = pg:FindFirstChild("MeteorRobloxGUI"); if oldPg then oldPg:Destroy() end end
    end)
    ...
    S.ChatConnections = {}
    for p, _ in pairs(S.ESPPool) do destroyESP(p) end
    ...
    pcall(function() if LP.Character then revertTallAnimations(LP.Character); disableGodMode() end end)
    pcall(function() toggleMapXray(false); toggleClearVision(false) end)
    pcall(function() Lighting.Ambient = originalAmbient; Lighting.OutdoorAmbient = originalOutdoor end)
    pcall(function() if getgenv().VoidFOVCircle then getgenv().VoidFOVCircle:Remove(); getgenv().VoidFOVCircle = nil end end)
end

if getgenv().VoidUtilityHubCleanup then getgenv().VoidUtilityHubCleanup() end
...
local function getHRP() local c = getChar(); return c and (c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("Torso") or c.PrimaryPart) end
local function getHum() local c = getChar(); return c and c:FindFirstChildOfClass("Humanoid") end

updateLocalNametag = function()
    ...
    if S.CustomNametag then
        if not bill then
            ...
            lbl.Size = UDim2.new(1, 0, 1, 0); lbl.Font = Enum.Font.GothamBold; lbl.TextColor3 = currentThemeColor
            ...
            if lbl then 
                ...
                if lbl.TextColor3 ~= currentThemeColor then lbl.TextColor3 = currentThemeColor end
            end
        end
    else
...
processUISpoofText = function(obj)
    local txt = obj.Text
    local newName = (S.CustomUIText and S.CustomUIText ~= "") and S.CustomUIText or S.SessionSpoofName
    local newText = txt
    ...
    if startPos then
        ...
        local lowerDisp = string.lower(LP.DisplayName)
...
applyTallAnimations = function(char)
...
revertTallAnimations = function(char)
...
applyCustomIdle = function(char)
    ...
    if S.CustomIdleAnim then
        ...
        if not animId or animId == "" then animId = animPresets[math.random(1, #animPresets)]; S.CustomIdleID = animId end
        ...
        if success and track then
            ...
            notify("Failed to load animation. Fallback applied.", Color3.fromRGB(218, 38, 38))
...
applyGodMode = function(character)
...
disableGodMode = function()
    ...
    local char = LP.Character; local humanoid = char and char:FindFirstChildOfClass("Humanoid")
...
toggleFloat = function(v)
    S.Float = v; local char = LP.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if v and hrp then
...
toggleClearVision = function(v)
    S.ClearVision = v
    if v then
        if S.OriginalFogEnd == nil then S.OriginalFogEnd = Lighting.FogEnd end
        ...
        if S.OriginalFogEnd ~= nil then Lighting.FogEnd = S.OriginalFogEnd; S.OriginalFogEnd = nil end
...
toggleGraphicsReducer = function(v)
    S.GraphicsReducer = v
    if v then
    ...
    pcall(function()
        local conn = game:GetService("GuiService").ErrorMessageChanged:Connect(function()
            if S.AutoRejoin then
                if notify then notify("Kicked from server! Rejoining in 5 seconds...", Color3.fromRGB(218, 38, 38)) end
                task.wait(5); TeleportService:Teleport(game.PlaceId, LP)
            end
        end)
...
local function teleportToPlace(placeId)
    notify("Teleporting to Place " .. placeId .. "...", Color3.fromRGB(218, 170, 42)); setupAutoReinject()
...
local function checkFriendship(userId)
    ...
    local isFr = false; pcall(function() isFr = LP:IsFriendsWith(userId) end)
    friendshipCache[userId] = isFr; return isFr
end

local function teleportToHRP(targetHRP)
    local myChar = LP.Character; local myHRP = getHRP(); local myHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
...
local function spectatePlayer(target)
    ...
    if target and target.Character then
        ...
        if hum then Camera.CameraType = Enum.CameraType.Watch; Camera.CameraSubject = hum; return end
    end
    Camera.CameraType = Enum.CameraType.Custom
    local myChar = LP.Character; local myHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
    if myHum then Camera.CameraSubject = myHum end
    currentSpectateTarget = nil
end

local function resetCameraToSelf() spectatePlayer(nil) end

local function enableFreecam()
    if isFreecam then return end; isFreecam = true
    local char = LP.Character; local hrp = getHRP()
    if hrp then hrp.Anchored = true end
    Camera.CameraType = Enum.CameraType.Scriptable; freecamBasePos = Camera.CFrame
    ...
    freecamConnection = RunService.RenderStepped:Connect(function(dt)
        if UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then UserInputService.MouseBehavior = Enum.MouseBehavior.LockCurrentPosition
        ...
        if moveDir.Magnitude > 0 then freecamBasePos = freecamBasePos + moveDir.Unit * ((S.FreecamSpeed or 40) * dt) end
        Camera.CFrame = CFrame.new(freecamBasePos.Position) * cameraRot
    end)
end

local function disableFreecam()
    if not isFreecam then return end; isFreecam = false
    if freecamConnection then freecamConnection:Disconnect(); freecamConnection = nil end
    if freecamInputBeganConn then freecamInputBeganConn:Disconnect(); freecamInputBeganConn = nil end
    if freecamInputEndedConn then freecamInputEndedConn:Disconnect(); freecamInputEndedConn = nil end
        ...
        if res then
            ...
            if data and data.data then
                ...
                if #possibleServers > 0 then
                    ...
                    notify("Found server! Teleporting...", Color3.fromRGB(50, 195, 75)); setupAutoReinject(); task.wait(0.5)
                    TeleportService:TeleportToPlaceInstance(placeId, chosen.id, LP)
                else notify("No suitable server found!", Color3.fromRGB(218, 38, 38)) end
            else notify("No server data in list!", Color3.fromRGB(218, 38, 38)) end
        else notify("Failed to query Roblox servers proxy!", Color3.fromRGB(218, 38, 38)) end
    end)
end

local function teleportToRandom() serverHop("Asc") end
local function teleportToLowestPop() serverHop("Asc") end
local function teleportToHighestPop() serverHop("Desc") end

local function runExternalScript(name, url, optionalPlaceId)
    if optionalPlaceId and game.PlaceId ~= optionalPlaceId then notify("This script requires PlaceId " .. tostring(optionalPlaceId), Color3.fromRGB(218, 38, 38)); return end
    notify("Loading script: " .. name .. "...", Color3.fromRGB(218, 170, 42))
    task.spawn(function()
        local ok, err = pcall(function() loadstring(game:HttpGet(url))() end)
        if ok then notify("Loaded " .. name .. " successfully!", Color3.fromRGB(50, 195, 75))
        else notify("Failed to load " .. name .. ": " .. tostring(err), Color3.fromRGB(218, 38, 38)) end
    end)
end

local function logMessage(sender, text, color)
    local logObj = { message = string.format("[%s]: %s", sender, text), messageType = Enum.MessageType.MessageOutput, timestamp = os.date("%H:%M:%S") }
    table.insert(consoleLogs, logObj)
    if #consoleLogs > 500 then table.remove(consoleLogs, 1) end
    if activeConsoleFeed then activeConsoleFeed:AddEntry(logObj.message, color or Color3.fromRGB(200, 200, 200)) end
end
    ...
    pcall(function()
        ...
        for _, log in ipairs(history) do
            ...
            if existingLog then
                ...
                for idx, item in ipairs(consoleLogs) do if item == existingLog then table.remove(consoleLogs, idx); break end end
                table.insert(consoleLogs, existingLog)
            else
                local logObj = { message = msg, messageType = msgType, timestamp = timestamp, count = 1 }
                table.insert(consoleLogs, logObj); consoleLogsMap[key] = logObj
                if #consoleLogs > 500 then local removed = table.remove(consoleLogs, 1); if removed then consoleLogsMap[removed.messageType.Value .. "_" .. removed.message] = nil end end
            end
        end
    end)
    ...
    if useModern then
        pcall(function()
            local modernCon = chatService.MessageReceived:Connect(function(msgObj)
                ...
                if activeChatFeed then activeChatFeed:AddEntry(string.format("[%s] [%s]: %s", timestamp, speaker, text), log.Color) end
                appendToChatLogFile(timestamp, speaker, text)
                if S.ToastChatEnabled and speaker ~= LP.DisplayName then showToast(speaker .. ": " .. text, currentThemeColor) end
            end)
        ...
        pcall(function()
            for _, p in ipairs(Players:GetPlayers()) do
                S.ChatConnections[p] = p.Chatted:Connect(function(msg)
                    ...
                    table.insert(S.ChatHistory, log)
                    if #S.ChatHistory > 200 then table.remove(S.ChatHistory, 1) end
                    if activeChatFeed then activeChatFeed:AddEntry(string.format("[%s] [%s]: %s", timestamp, speaker, msg), log.Color) end
                    appendToChatLogFile(timestamp, speaker, msg)
                    if S.ToastChatEnabled and p ~= LP then showToast(speaker .. ": " .. msg, currentThemeColor) end
                end)
            end
...
local function createOverhead(p)
    if p == LP then return end; local char = p.Character; local head = char and char:WaitForChild("Head", 3)
...
local function refreshOverheads()
    ...
    for _, p in ipairs(Players:GetPlayers()) do if p ~= LP and p.Character then createOverhead(p) end end
end

local function saveFavorites() pcall(function() if writefile then writefile("utility_hub_favorites.json", HttpService:JSONEncode(S.FavoriteMaps)) end end) end
local function loadFavorites()
    pcall(function()
        if readfile and isfile and isfile("utility_hub_favorites.json") then
            local data = readfile("utility_hub_favorites.json"); S.FavoriteMaps = HttpService:JSONDecode(data) or {}
        end
    end)
end
loadFavorites()
    ...
    pcall(function()
        if writefile then
            ...
            for k, v in pairs(S) do
                if type(v) == "boolean" or type(v) == "number" or type(v) == "string" then configData[k] = v
                elseif typeof(v) == "EnumItem" then configData[k] = {__type = "EnumItem", Value = tostring(v)} end
            end
...
local function loadConfig()
    pcall(function()
        if readfile and isfile and isfile("utility_hub_config.json") then
            local data = readfile("utility_hub_config.json"); local configData = HttpService:JSONDecode(data)
            if configData then
                for k, v in pairs(configData) do
                    if type(v) == "table" and v.__type == "EnumItem" then
                        local enumType, enumName = v.Value:match("^Enum%.([^%.]+)%.([^%.]+)$")
                        if enumType and enumName and Enum[enumType] and Enum[enumType][enumName] then S[k] = Enum[enumType][enumName] end
                    else S[k] = v end
                end
            end
        end
...
setupAutoRejoin()
setupAutoReinject()
pcall(function() if setfpscap and S.FPSCap then setfpscap(S.FPSCap) end end)

-- ──────────────────────────────────────────────────────────────
--  UI & RUNTIME INITIALIZATION (Encapsulated to bypass 200 local limit)
-- ──────────────────────────────────────────────────────────────
...
currentThemeColor = themeColors[S.ThemeColor or "Purple"] or themeColors["Purple"]
...
if not menuBlur then menuBlur = Instance.new("BlurEffect"); menuBlur.Name = "WeAreSkiddingBlur"; menuBlur.Size = 0; menuBlur.Enabled = false; menuBlur.Parent = Lighting end

local function updateMenuBlur()
    if not uiVisible then
        TweenService:Create(menuBlur, TweenInfo.new(0.25), {Size = 0}):Play()
        task.delay(0.25, function() if not uiVisible then menuBlur.Enabled = false end end)
        return
    end
    if activeTab == "Settings" then
        menuBlur.Enabled = true; TweenService:Create(menuBlur, TweenInfo.new(0.25), {Size = 16}):Play()
    else
        TweenService:Create(menuBlur, TweenInfo.new(0.25), {Size = 0}):Play()
        task.delay(0.25, function() if activeTab == "Modules" or not uiVisible then menuBlur.Enabled = false end end)
    end
end

local tabButtons, windows, moduleButtons, floatingWindows = {}, {}, {}, {}
local hudWatermark, hudCoords, hudArrayListFrame, hudServerAge = nil, nil, nil, nil

updateHUDArrayList = function()
    ...
    if not uiVisible then isVisible = S.HUDArrayList and S.HUDArrayListOutside end
    if not isVisible then hudArrayListFrame.Visible = false return end
    local activeMods = {}
    for modName, item in pairs(moduleButtons) do if item.IsActive and item.IsActive() then table.insert(activeMods, modName) end end
    ...
    if #activeMods == 0 then hudArrayListFrame.Visible = false
        ...
        for _, modName in ipairs(activeMods) do
            ...
            lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 9; lbl.TextColor3 = currentThemeColor
            ...
            accent.BackgroundColor3 = currentThemeColor; accent.BorderSizePixel = 0; accent.Parent = lbl
        end
    end
end

task.spawn(function() while true do task.wait(0.2); pcall(updateHUDArrayList) end end)

local function applyThemeColor(colorName)
    local col = themeColors[colorName] or themeColors["Purple"]
    currentThemeColor = col; S.ThemeColor = colorName
    ...
    for name, btn in pairs(tabButtons) do if name == activeTab then btn.TextColor3 = col end end
    if hudWatermark then hudWatermark.TextColor3 = col end
    task.defer(updateHUDArrayList); task.defer(updateLocalNametag)
end
...
topStroke.Color = Color3.fromRGB(30, 30, 30); topStroke.Thickness = 1; topStroke.Parent = topBar
...
topTitle.Font = Enum.Font.GothamBold; topTitle.TextSize = 11; topTitle.TextColor3 = currentThemeColor
...
showToast = function(message, color)
    ...
    local stroke = Instance.new("UIStroke"); stroke.Color = color or currentThemeColor; stroke.Thickness = 1.2; stroke.Parent = toast
...
notify = function(msg, color) showToast(msg, color) end
...
local function makeResizable(frame, handle)
    ...
    local function update(input) local delta = input.Position - dragStart; local newWidth = math.max(120, startSize.X.Offset + delta.X); local newHeight = math.max(50, startSize.Y.Offset + delta.Y); frame.Size = UDim2.new(0, newWidth, 0, newHeight) end
    handle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
...
local function addToggleOption(parent, name, defaultVal, callback)
    ...
    btn.BackgroundColor3 = defaultVal and currentThemeColor or Color3.fromRGB(55, 55, 55); btn.BorderSizePixel = 0; btn.Text = ""; btn.Parent = row
    local btnCorner = Instance.new("UICorner"); btnCorner.CornerRadius = UDim.new(0, 7); btnCorner.Parent = btn
    local btnStroke = Instance.new("UIStroke"); btnStroke.Thickness = 1; btnStroke.Color = defaultVal and currentThemeColor or Color3.fromRGB(80, 80, 80); btnStroke.Parent = btn
...
local function addSliderOption(parent, name, min, max, defaultVal, callback)
    ...
    valLabel.BackgroundTransparency = 1; valLabel.Font = Enum.Font.GothamBold; valLabel.TextSize = 10; valLabel.TextColor3 = currentThemeColor
    ...
    slideFill.Size = UDim2.new(startPct, 0, 1, 0); slideFill.BackgroundColor3 = currentThemeColor; slideFill.BorderSizePixel = 0; slideFill.Parent = slideBg; table.insert(themeFills, slideFill)
    ...
    local moveCon = UserInputService.InputChanged:Connect(function(input) if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then updateSlider(input) end end)
...
local function addKeybindOption(parent, name, defaultKey, callback)
    ...
    bindBtn.TextColor3 = currentThemeColor; bindBtn.Text = (defaultKey and defaultKey ~= Enum.KeyCode.Unknown) and defaultKey.Name or "[none]"
    ...
    local con = UserInputService.InputBegan:Connect(function(input)
        if listening then
            if input.UserInputType == Enum.UserInputType.Keyboard then
                local key = input.KeyCode
                if key == Enum.KeyCode.Escape then listening = false; bindBtn.Text = (currentKey and currentKey ~= Enum.KeyCode.Unknown) and currentKey.Name or "[none]"; bindBtn.TextColor3 = currentThemeColor; return
                elseif key == Enum.KeyCode.Backspace or key == Enum.KeyCode.Delete then listening = false; currentKey = Enum.KeyCode.Unknown; bindBtn.Text = "[none]"; bindBtn.TextColor3 = currentThemeColor; callback(Enum.KeyCode.Unknown); return end
                if UserInputService:GetFocusedTextBox() then return end
                listening = false; currentKey = key; bindBtn.Text = (key and key ~= Enum.KeyCode.Unknown) and key.Name or "[none]"; bindBtn.TextColor3 = currentThemeColor
            ...
            elseif input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
                task.wait(0.05)
                if listening then listening = false; bindBtn.Text = (currentKey and currentKey ~= Enum.KeyCode.Unknown) and currentKey.Name or "[none]"; bindBtn.TextColor3 = currentThemeColor end
            end
        end
    end)
    table.insert(S.Connections, con)
...
local function addButtonOption(parent, name, callback)
    ...
    btn.BackgroundColor3 = currentThemeColor; btn.BorderSizePixel = 0; btn.Font = Enum.Font.GothamBold; btn.TextSize = 9
...
local function addSectionHeader(parent, title)
    ...
    text.BackgroundTransparency = 1; text.Font = Enum.Font.GothamBold; text.TextSize = 10; text.TextColor3 = currentThemeColor
...
local function getOrCreateWindow(catName, defaultX, defaultY)
    ...
    local header = Instance.new("TextButton"); header.Size = UDim2.new(1, 0, 0, 22); header.BackgroundColor3 = currentThemeColor
    ...
    windows[catName] = { Frame = win, List = list, Layout = listLayout }; return windows[catName]
end
...
local function createFloatingWindow(title, width, height, defaultX, defaultY)
    ...
    resizeHandle.Position = UDim2.new(1, -6, 1, -6); resizeHandle.BackgroundColor3 = currentThemeColor; resizeHandle.BackgroundTransparency = 0.3
    ...
    makeDraggable(win, header); table.insert(floatingWindows, win)
...
local function registerModule(catName, name, defaultX, defaultY, isToggle, defaultState, callback, populateOptionsFunc, useSeparateWindow, winWidth, winHeight)
    ...
    local function updateColor() btn.TextColor3 = (isToggle and active) and Color3.fromRGB(100, 240, 100) or Color3.fromRGB(200, 200, 200); task.defer(updateHUDArrayList) end
    ...
    btn.MouseLeave:Connect(function() updateBg() end)
    btn.MouseButton1Click:Connect(function()
        if isToggle then active = not active; updateColor(); if callback then callback(active) end
    ...
    local function toggleDrawer()
        ...
        if useSeparateWindow and floatingWin then
            ...
            if gear then
                local targetRot = floatingWin.Visible and 90 or 0; local targetCol = floatingWin.Visible and currentThemeColor or Color3.fromRGB(120, 120, 120)
                TweenService:Create(gear, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = targetRot, TextColor3 = targetCol}):Play()
            end
        elseif drawer then
            local open = not drawer.Visible; isTweening = true
            if open then
                drawer.AutomaticSize = Enum.AutomaticSize.None; drawer.ClipsDescendants = true; drawer.Size = UDim2.new(1, 0, 0, 0)
                drawer.Visible = true; updateBg(); local optionsFrame = drawer:FindFirstChild("optionsFrame"); local targetHeight = 0
                if optionsFrame then for _, child in ipairs(optionsFrame:GetChildren()) do if child:IsA("Frame") then targetHeight = targetHeight + child.Size.Y.Offset + 3 end end end
                targetHeight = targetHeight + 6
                if gear then TweenService:Create(gear, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 90, TextColor3 = currentThemeColor}):Play() end
                local tween = TweenService:Create(drawer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, targetHeight)})
                tween.Completed:Connect(function() drawer.AutomaticSize = Enum.AutomaticSize.Y; isTweening = false end); tween:Play(); S.currentOptionsModule = name
            else
                drawer.AutomaticSize = Enum.AutomaticSize.None; drawer.ClipsDescendants = true
                if gear then TweenService:Create(gear, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 0, TextColor3 = Color3.fromRGB(120, 120, 120)}):Play() end
                local tween = TweenService:Create(drawer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, 0)})
                tween.Completed:Connect(function() drawer.Visible = false; updateBg(); isTweening = false end); tween:Play()
    ...
    if populateOptionsFunc then
        if useSeparateWindow then
            local targetX = defaultX + 150; if targetX > 900 then targetX = defaultX - (winWidth or 230) - 10 end
            local winFrame, contentFrame = createFloatingWindow(name, winWidth or 230, winHeight or 220, targetX, defaultY)
            floatingWin = winFrame; populateOptionsFunc(contentFrame); adjustWindowSizeToContent(winFrame, contentFrame)
            winFrame:GetPropertyChangedSignal("Visible"):Connect(function()
                updateBg(); if gear then
                    local targetRot = winFrame.Visible and 90 or 0; local targetCol = winFrame.Visible and currentThemeColor or Color3.fromRGB(120, 120, 120)
                    TweenService:Create(gear, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = targetRot, TextColor3 = targetCol}):Play()
                end
            end)
        else
            drawer = Instance.new("Frame"); drawer.Name = "drawer"; drawer.Size = UDim2.new(1, 0, 0, 0); drawer.AutomaticSize = Enum.AutomaticSize.Y
            drawer.Position = UDim2.new(0, 0, 0, 20); drawer.BackgroundColor3 = Color3.fromRGB(15, 15, 15); drawer.BackgroundTransparency = 0.3
            drawer.BorderSizePixel = 0; drawer.Visible = false; drawer.Parent = container
            local accent = Instance.new("Frame"); accent.Size = UDim2.new(0, 2, 1, 0); accent.Position = UDim2.new(0, 2, 0, 0)
            accent.BackgroundColor3 = currentThemeColor; accent.BorderSizePixel = 0; accent.Parent = drawer; table.insert(themeFills, accent)
            local optionsFrame = Instance.new("Frame"); optionsFrame.Name = "optionsFrame"; optionsFrame.Size = UDim2.new(1, -8, 0, 0)
            optionsFrame.AutomaticSize = Enum.AutomaticSize.Y; optionsFrame.Position = UDim2.new(0, 8, 0, 0); optionsFrame.BackgroundTransparency = 1
            optionsFrame.BorderSizePixel = 0; optionsFrame.Parent = drawer
            local drawerLayout = Instance.new("UIListLayout"); drawerLayout.Padding = UDim.new(0, 3); drawerLayout.Parent = optionsFrame
            local uipadding = Instance.new("UIPadding"); uipadding.PaddingTop = UDim.new(0, 2); uipadding.PaddingBottom = UDim.new(0, 4); uipadding.Parent = optionsFrame
            populateOptionsFunc(optionsFrame)
        end
        gear = Instance.new("TextButton"); gear.Size = UDim2.new(0, 20, 0, 20); gear.AnchorPoint = Vector2.new(0.5, 0.5); gear.Position = UDim2.new(1, -10, 0.5, 0)
        gear.BackgroundTransparency = 1; gear.Font = Enum.Font.Gotham; gear.TextSize = 10; gear.TextColor3 = Color3.fromRGB(120, 120, 120)
        gear.Text = ">"; gear.Parent = btn; gear.MouseButton1Click:Connect(toggleDrawer)
    end
    btn.MouseButton2Click:Connect(toggleDrawer)
    moduleButtons[name] = { 
        Button = btn, Container = container, Update = updateColor, 
        SetActive = function(val) if isToggle and active ~= val then active = val; updateColor(); if callback then callback(val) end end end,
...
local Win = {
    ...
    ResetAllToggles = function() for _, mod in pairs(moduleButtons) do if mod.IsActive and mod.IsActive() then mod.SetActive(false) end end end,
    SetAutoReinject = function(self, val, callback) end, SetOnClose = function(self, callback) end
}

local searchWin = getOrCreateWindow("Search", 860, 50)
local searchBox = Instance.new("TextBox"); searchBox.Size = UDim2.new(1, -10, 0, 22); searchBox.Position = UDim2.new(0, 5, 0, 5)
searchBox.BackgroundColor3 = Color3.fromRGB(30, 30, 30); searchBox.BorderSizePixel = 0; searchBox.Font = Enum.Font.Gotham; searchBox.TextSize = 9
searchBox.TextColor3 = Color3.fromRGB(240, 240, 240); searchBox.PlaceholderText = "Search modules..."; searchBox.Text = ""
searchBox.ClearTextOnFocus = false; searchBox.Parent = searchWin.List
local searchStroke = Instance.new("UIStroke"); searchStroke.Color = Color3.fromRGB(50, 50, 50); searchStroke.Parent = searchBox
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for modName, item in pairs(moduleButtons) do
        if text == "" or modName:lower():find(text) then item.Container.Visible = true else item.Container.Visible = false end
    end
end)

local navBar = Instance.new("Frame"); navBar.Size = UDim2.new(0, 600, 1, 0); navBar.Position = UDim2.new(0.5, -300, 0, 0)
navBar.BackgroundTransparency = 1; navBar.Parent = topBar
local navLayout = Instance.new("UIListLayout"); navLayout.FillDirection = Enum.FillDirection.Horizontal; navLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
navLayout.VerticalAlignment = Enum.VerticalAlignment.Center; navLayout.Padding = UDim.new(0, 16); navLayout.Parent = navBar

local settingsPanel, settingsContent = nil, nil
local function selectTab(tabName)
    activeTab = tabName
    for name, btn in pairs(tabButtons) do
        if name == tabName then btn.TextColor3 = currentThemeColor; btn.Font = Enum.Font.GothamBold
        else btn.TextColor3 = Color3.fromRGB(200, 200, 200); btn.Font = Enum.Font.Gotham end
    end
    local isModules = (tabName == "Modules")
    for catName, win in pairs(windows) do win.Frame.Visible = isModules end
    for _, win in ipairs(floatingWindows) do if isModules then win.Visible = win:GetAttribute("UserOpen") == true else win.Visible = false end end
    if settingsPanel then settingsPanel.Visible = (tabName == "Settings") end
    updateMenuBlur()
end

hudWatermark = Instance.new("TextLabel"); hudWatermark.Size = UDim2.new(0, 240, 0, 20); hudWatermark.Position = UDim2.new(0, 10, 0, 30)
hudWatermark.BackgroundTransparency = 1; hudWatermark.Font = Enum.Font.GothamBold; hudWatermark.TextSize = 12; hudWatermark.TextColor3 = currentThemeColor
hudWatermark.TextXAlignment = Enum.TextXAlignment.Left; hudWatermark.Text = "WeAreSkidding <font color='#ffffff'>On Roblox</font>"
hudWatermark.RichText = true; hudWatermark.ZIndex = 10; hudWatermark.Visible = S.HUDWatermark; hudWatermark.Parent = screenGui; table.insert(themeTexts, hudWatermark)

hudCoords = Instance.new("TextLabel"); hudCoords.Size = UDim2.new(0, 240, 0, 20); hudCoords.Position = UDim2.new(0, 10, 0, 46)
hudCoords.BackgroundTransparency = 1; hudCoords.Font = Enum.Font.Code; hudCoords.TextSize = 10; hudCoords.TextColor3 = Color3.fromRGB(200, 200, 200)
hudCoords.TextXAlignment = Enum.TextXAlignment.Left; hudCoords.Text = "XYZ: 0.0, 0.0, 0.0"; hudCoords.ZIndex = 10; hudCoords.Visible = S.HUDCoords; hudCoords.Parent = screenGui

hudServerAge = Instance.new("TextLabel"); hudServerAge.Size = UDim2.new(0, 240, 0, 20); hudServerAge.Position = UDim2.new(0, 10, 0, 62)
hudServerAge.BackgroundTransparency = 1; hudServerAge.Font = Enum.Font.Code; hudServerAge.TextSize = 10; hudServerAge.TextColor3 = Color3.fromRGB(200, 200, 200)
hudServerAge.TextXAlignment = Enum.TextXAlignment.Left; hudServerAge.Text = "Server Age: 0s"; hudServerAge.ZIndex = 10; hudServerAge.Visible = S.ServerAgeHUD; hudServerAge.Parent = screenGui

hudArrayListFrame = Instance.new("Frame"); hudArrayListFrame.Size = UDim2.new(0, 120, 0, 0)
hudArrayListFrame.Position = UDim2.new(0, S.HUDArrayListX or 10, 0, S.HUDArrayListY or 70)
hudArrayListFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15); hudArrayListFrame.BackgroundTransparency = 0.85; hudArrayListFrame.BorderSizePixel = 0
hudArrayListFrame.Active = true; hudArrayListFrame.ZIndex = 10; hudArrayListFrame.AutomaticSize = Enum.AutomaticSize.Y
hudArrayListFrame.Visible = S.HUDArrayList; hudArrayListFrame.Parent = screenGui

local hudCorner = Instance.new("UICorner"); hudCorner.CornerRadius = UDim.new(0, 4); hudCorner.Parent = hudArrayListFrame
...
hudArrayListFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        dragging = true; dragStart = input.Position; startPos = hudArrayListFrame.Position
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                dragging = false; S.HUDArrayListX = hudArrayListFrame.Position.X.Offset; S.HUDArrayListY = hudArrayListFrame.Position.Y.Offset; saveConfig()
            end
        end)
    end
end)
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        hudArrayListFrame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
    end
end)
...
local function createPanel(title, width, height)
    ...
    local winCorner = Instance.new("UICorner"); winCorner.CornerRadius = UDim.new(0, 6); winCorner.Parent = win
    local header = Instance.new("Frame"); header.Size = UDim2.new(1, 0, 0, 22); header.BackgroundColor3 = currentThemeColor; header.BorderSizePixel = 0; header.Parent = win
...
addButtonOption(settingsContent, "Apply legit closet profile", function()
    ...
    saveConfig(); notify("Closet Legit profile applied!", Color3.fromRGB(46, 204, 113))
end)
addButtonOption(settingsContent, "Apply blatant flight profile", function()
    ...
    saveConfig(); notify("Blatant profile applied!", Color3.fromRGB(241, 196, 15))
end)
addButtonOption(settingsContent, "Apply rage combat profile", function()
    ...
    saveConfig(); notify("Rage profile applied!", Color3.fromRGB(218, 38, 38))
end)

addSectionHeader(settingsContent, "UI & HUD Customization")
addDropdownOption(settingsContent, "Interface Theme Color", {"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, table.find({"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, S.ThemeColor) or 1, function(_, opt) applyThemeColor(opt); saveConfig() end)
addKeybindOption(settingsContent, "Menu Toggle Keybind", S.UIToggleKey or Enum.KeyCode.RightControl, function(k) S.UIToggleKey = k; saveConfig(); notify("UI Toggle Keybind set to: " .. k.Name, Color3.fromRGB(50, 195, 75)) end)
addToggleOption(settingsContent, "Show Toasts Enabled", S.ToastEnabled, function(v) S.ToastEnabled = v; saveConfig() end)
addToggleOption(settingsContent, "Display Client Watermark", S.HUDWatermark, function(v) S.HUDWatermark = v; hudWatermark.Visible = v; saveConfig() end)
addToggleOption(settingsContent, "Display Player Coordinates", S.HUDCoords, function(v) S.HUDCoords = v; hudCoords.Visible = v; saveConfig() end)
addToggleOption(settingsContent, "Display Server Age HUD", S.ServerAgeHUD, function(v) S.ServerAgeHUD = v; hudServerAge.Visible = v; saveConfig() end)
addToggleOption(settingsContent, "Display Active ArrayList", S.HUDArrayList, function(v) S.HUDArrayList = v; updateHUDArrayList(); saveConfig() end)
addToggleOption(settingsContent, "Display active mods when outside of the main UI", S.HUDArrayListOutside, function(v) S.HUDArrayListOutside = v; updateHUDArrayList(); saveConfig() end)

addSectionHeader(settingsContent, "Targets & Input Settings")
addTextboxOption(settingsContent, "Specify Target / Friend", "Username", function(txt) if txt == "" then return end; notify("Target lock set to: " .. txt, Color3.fromRGB(50, 195, 75)) end)
addButtonOption(settingsContent, "Clear Current Friends Lists", function() notify("Friends lists reset", Color3.fromRGB(218, 38, 38)) end)
addTextboxOption(settingsContent, "Configure Macro Text", "Say something...", function(txt) S.MacroText = txt; saveConfig(); notify("Macro text configured!", Color3.fromRGB(50, 195, 75)) end)
addKeybindOption(settingsContent, "Trigger Macro Key", S.MacroKey or Enum.KeyCode.H, function(k) S.MacroKey = k; saveConfig(); notify("Macro trigger set to: " .. k.Name, Color3.fromRGB(50, 195, 75)) end)
addKeybindOption(settingsContent, "Panic Button (Disable All)", S.PanicKey or Enum.KeyCode.End, function(k) S.PanicKey = k; saveConfig(); notify("Panic Key set to: " .. k.Name, Color3.fromRGB(218, 38, 38)) end)
addKeybindOption(settingsContent, "Grab User ID (Hover Player)", S.UserIDGrabKey or Enum.KeyCode.K, function(k) S.UserIDGrabKey = k; saveConfig(); notify("UserID Grab set to: " .. k.Name, Color3.fromRGB(50, 195, 75)) end)

addSectionHeader(settingsContent, "Executor Capabilities")
...
addInfoRowOption(settingsContent, "Supported Functions", supportedFuncs .. " / " .. totalFuncs)
addInfoRowOption(settingsContent, "Executor Name", executorName)

addSectionHeader(settingsContent, "Config & Client Controls")
addTextboxOption(settingsContent, "Configuration Name", "utility_hub_config", function(txt) end)
addButtonOption(settingsContent, "Save Current Settings", function() saveConfig(); notify("Configuration saved successfully!", Color3.fromRGB(50, 195, 75)) end)
addButtonOption(settingsContent, "Load Stored Settings", function() loadConfig(); notify("Configuration loaded successfully!", Color3.fromRGB(50, 195, 75)) end)
addButtonOption(settingsContent, "Reset Settings to Default", function()
    Win:ResetAllToggles()
    S.WalkSpeed = 16; S.JumpPower = 50; S.InfJump = false; S.BHop = false; S.AirWalk = false; S.NoClip = false; S.Fly = false; S.FlySpeed = 60; S.ESPBoxes = false; S.ESPTracers = false; S.ESPNames = false; S.ESPHealth = false; S.ESPDistances = false; S.ESPTeamCheck = false; S.AimbotActive = false; S.AntiAFK = false; S.AutoRejoin = false; S.GravityEnabled = false; S.CustomGravity = 196.2; S.ThemeColor = "Purple"
    applyThemeColor("Purple"); saveConfig(); notify("All settings reset to default!", Color3.fromRGB(218, 38, 38))
end)
addButtonOption(settingsContent, "Destruct Client GUI Completely", function() cleanupAll() end)
...
for _, tabName in ipairs(tabs) do
    ...
    btn.Font = Enum.Font.Gotham; btn.TextSize = 12; btn.TextColor3 = (tabName == activeTab) and currentThemeColor or Color3.fromRGB(200, 200, 200)
    ...
    btn.MouseButton1Click:Connect(function() selectTab(tabName) end); tabButtons[tabName] = btn
end
selectTab("Modules")
...
local function getSpectateList() local list = {}; for _, p in ipairs(Players:GetPlayers()) do if p ~= LP then table.insert(list, p) end end; return list end
local function applySpectate() local list = getSpectateList(); if #list == 0 then notify("No targets in server to spectate", Color3.fromRGB(218, 38, 38)); return end; spectateIndex = math.clamp(spectateIndex, 1, #list); local target = list[spectateIndex]; if target then spectatePlayer(target) end end
...
registerModule("Render", "Skip Teammates", 440, 50, true, S.ESPTeamCheck, function(v) S.ESPTeamCheck = v; saveConfig() end)
...
local function rebuildFriends(scroll)
    ...
    task.spawn(function()
        ...
        for _, item in ipairs(onlineFriends) do
            ...
            local nameL = Instance.new("TextLabel"); nameL.Text = item.DisplayName or item.UserName; nameL.Font = Enum.Font.GothamBold; nameL.TextSize = math.clamp(math.round(7 * scale), 7, 24); nameL.TextColor3 = Color3.fromRGB(230, 230, 230); nameL.BackgroundTransparency = 1; nameL.Position = UDim2.new(0, 2, 0, 1 * scale); nameL.Size = UDim2.new(0.5, 0, 0, 10 * scale); nameL.TextXAlignment = Enum.TextXAlignment.Left; nameL.Parent = card
            ...
            local detL = Instance.new("TextLabel"); detL.Text = statusText; detL.Font = Enum.Font.Gotham; detL.TextSize = math.clamp(math.round(6 * scale), 7, 24); detL.TextColor3 = statusColor; detL.BackgroundTransparency = 1; detL.Position = UDim2.new(0, 2, 0, 11 * scale); detL.Size = UDim2.new(0.7, 0, 0, 10 * scale); detL.TextXAlignment = Enum.TextXAlignment.Left; detL.Parent = card
            if isInGame then
...
-- ──────────────────────────────────────────────────────────────
--  GHOST MODE & FLYING UTILITIES
-- ──────────────────────────────────────────────────────────────
function enableGhostMode()
    local myChar = LP.Character; local myHRP = myChar and (myChar:FindFirstChild("HumanoidRootPart") or myChar:FindFirstChild("Torso") or myChar.PrimaryPart)
    ...
    pcall(function()
        ...
        clone.Parent = Workspace; S.GhostDummy = clone
    end)
    ...
    S.Fly = true; flyOn(); S.NoClip = true; notify("Ghost state active: body parked", Color3.fromRGB(218, 170, 42))
end
...
function disableGhostMode()
    ...
    if myHRP and S.GhostCFrame then
        if not S.GhostTeleportToEnd then myHRP.CFrame = S.GhostCFrame; notify("Ghost returned to body origin", Color3.fromRGB(50, 195, 75))
        else notify("Teleported body to ghost position!", Color3.fromRGB(50, 195, 75)) end
        S.GhostCFrame = nil
    end
    ...
    S.Fly = false; flyOff(); S.NoClip = false
end
...
local function updateFlyVelocity()
    ...
    local dir = Vector3.zero; local cf = Camera.CFrame; local fwd = cf.LookVector
...
local function isPartVisible(part, char)
    ...
    local origin = Camera.CFrame.Position; local destination = part.Position; local direction = destination - origin
    if direction.Magnitude == 0 then return true end
    visRaycastParams.FilterDescendantsInstances = {LP.Character, char}
    local result = Workspace:Raycast(origin, direction, visRaycastParams); return result == nil
end
...
getAimbotTarget = function()
    local bestTarget = nil; local closestDist = S.AimbotFOV; local center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        local part = getAimbotTargetPart(char); if not part then continue end
...
fovCircle.Thickness = 1; fovCircle.Color = Color3.fromRGB(218, 38, 38); fovCircle.Filled = false; fovCircle.Transparency = 1
        ...
        if S.BHop and myHRP and myHum then
            ...
            if moveDir.Magnitude > 0.1 then
                ...
                if myHum.FloorMaterial ~= Enum.Material.Air then
                    ...
                    local currentVel = myHRP.AssemblyLinearVelocity; local horizontalVel = Vector3.new(currentVel.X, 0, currentVel.Z)
                    local newHorizontal = horizontalVel + (moveDir * 4)
                    if newHorizontal.Magnitude > 120 then newHorizontal = newHorizontal.Unit * 120 end
                    myHRP.AssemblyLinearVelocity = Vector3.new(newHorizontal.X, currentVel.Y, newHorizontal.Z)
                else
                    local currentVel = myHRP.AssemblyLinearVelocity; local horizontalVel = Vector3.new(currentVel.X, 0, currentVel.Z); local speed = horizontalVel.Magnitude
                    if speed > 0.1 then local newHorizontal = (horizontalVel + moveDir * 2).Unit * speed; myHRP.AssemblyLinearVelocity = Vector3.new(newHorizontal.X, currentVel.Y, newHorizontal.Z)
                    else myHRP.AssemblyLinearVelocity = Vector3.new((moveDir * 16).X, currentVel.Y, (moveDir * 16).Z) end
                    if S.BHopAutoStrafe then
                        if UserInputService:IsKeyDown(Enum.KeyCode.A) then Camera.CFrame = Camera.CFrame * CFrame.Angles(0, math.rad(1.0), 0)
                        elseif UserInputService:IsKeyDown(Enum.KeyCode.D) then Camera.CFrame = Camera.CFrame * CFrame.Angles(0, math.rad(-1.0), 0) end
                    end
                end
            end
        end
