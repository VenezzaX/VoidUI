--[[
    ╔══════════════════════════════════════════════════════════════╗
    ║               METEOR HUB FOR ROBLOX (CUSTOM GUI)             ║
    ║          Built from scratch - No external UI library         ║
    ║          Meteor Client aesthetics, dragging, collapse        ║
    ║          Dynamic inline configuration drawers & search       ║
    ╚══════════════════════════════════════════════════════════════╝
]]
    ...
    local code = [[
        ...
        if isfile and isfile("Script.lua") then
            ...
            local ok, err = pcall(function()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/VenezzaX/VoidUI/refs/heads/main/client.lua", true))()
            end)
            if not ok then
...
local function updateMenuBlur()
    ...
    local needsBlur = table.find({"Config", "GUI", "HUD", "Friends", "Macros", "Profiles"}, activeTab) ~= nil
    if needsBlur then
...
local function addToggleOption(parent, name, defaultVal, callback)
    ...
    btn.Position = UDim2.new(1, -14, 0.5, -5)
    btn.BackgroundColor3 = defaultVal and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 50)
    ...
    btn.MouseButton1Click:Connect(function()
        active = not active
        btn.BackgroundColor3 = active and Color3.fromRGB(46, 204, 113) or Color3.fromRGB(50, 50, 50)
        callback(active)
    end)
...
local function addSliderOption(parent, name, min, max, defaultVal, callback)
    ...
    slideBg.Size = UDim2.new(1, -8, 0, 4)
    slideBg.Position = UDim2.new(0, 4, 0, 14)
...
local function addKeybindOption(parent, name, defaultKey, callback)
    ...
    bindBtn.Size = UDim2.new(0, 50, 0, 12)
    bindBtn.Position = UDim2.new(1, -54, 0.5, -6)
...
local function registerModule(catName, name, defaultX, defaultY, isToggle, defaultState, callback, populateOptionsFunc, useSeparateWindow, winWidth, winHeight)
    ...
    if populateOptionsFunc then
        if useSeparateWindow then
            ...
            drawer.BackgroundColor3 = Color3.fromRGB(15, 15, 15, 120)
            ...
            uipadding.Parent = drawer

            populateOptionsFunc(drawer)
        end
    end
...
local configPanel, configContent = nil, nil
local guiPanel, guiContent = nil, nil
local hudPanel, hudContent = nil, nil
local friendsPanel, friendsContent = nil, nil
local macrosPanel, macrosContent = nil, nil
local profilesPanel, profilesContent = nil, nil
...
local function selectTab(tabName)
    ...
    if configPanel then configPanel.Visible = (tabName == "Config") end
    if guiPanel then guiPanel.Visible = (tabName == "GUI") end
    if hudPanel then hudPanel.Visible = (tabName == "HUD") end
    if friendsPanel then friendsPanel.Visible = (tabName == "Friends") end
    if macrosPanel then macrosPanel.Visible = (tabName == "Macros") end
    if profilesPanel then profilesPanel.Visible = (tabName == "Profiles") end
...
-- 1. Create Config Panel
configPanel, configContent = createPanel("Config Manager", 230, 150)
addTextboxOption(configContent, "Configuration Name", "utility_hub_config", function(txt)
    -- Reserved
end)
addButtonOption(configContent, "Save Current Settings", function()
    saveConfig()
    notify("Configuration saved successfully!", Color3.fromRGB(50, 195, 75))
end)
addButtonOption(configContent, "Load Stored Settings", function()
    loadConfig()
    notify("Configuration loaded successfully!", Color3.fromRGB(50, 195, 75))
end)
addButtonOption(configContent, "Reset Settings to Default", function()
    Win:ResetAllToggles()
    S.WalkSpeed = 16
    S.JumpPower = 50
    S.InfJump = false
...
adjustWindowSizeToContent(configPanel, configContent)

-- 2. Create GUI Panel
guiPanel, guiContent = createPanel("GUI Settings", 230, 160)
addDropdownOption(guiContent, "Interface Theme Color", {"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, table.find({"Purple", "Red", "Green", "Blue", "Yellow", "Cyan", "Pink", "Orange"}, S.ThemeColor) or 1, function(_, opt)
    applyThemeColor(opt)
    saveConfig()
end)
addKeybindOption(guiContent, "Menu Toggle Keybind", S.UIToggleKey or Enum.KeyCode.RightControl, function(k)
    S.UIToggleKey = k
    saveConfig()
    notify("UI Toggle Keybind set to: " .. k.Name, Color3.fromRGB(50, 195, 75))
end)
addToggleOption(guiContent, "Show Toasts Enabled", S.ToastEnabled, function(v)
    S.ToastEnabled = v
    saveConfig()
end)
addButtonOption(guiContent, "Destruct Client GUI Completely", function()
    cleanupAll()
end)
adjustWindowSizeToContent(guiPanel, guiContent)

-- 3. Create HUD Panel
hudPanel, hudContent = createPanel("HUD Customization", 230, 160)
addToggleOption(hudContent, "Display Client Watermark", S.HUDWatermark, function(v)
    S.HUDWatermark = v
    hudWatermark.Visible = v
    saveConfig()
end)
addToggleOption(hudContent, "Display Player Coordinates", S.HUDCoords, function(v)
    S.HUDCoords = v
    hudCoords.Visible = v
    saveConfig()
end)
addToggleOption(hudContent, "Display Active ArrayList", S.HUDArrayList, function(v)
    S.HUDArrayList = v
    updateHUDArrayList()
    saveConfig()
end)
adjustWindowSizeToContent(hudPanel, hudContent)

-- 4. Create Friends Panel
friendsPanel, friendsContent = createPanel("Client Targets & Friends", 230, 160)
addTextboxOption(friendsContent, "Specify Target / Friend", "Username", function(txt)
    if txt == "" then return end
    notify("Target lock set to: " .. txt, Color3.fromRGB(50, 195, 75))
end)
addButtonOption(friendsContent, "Clear Current Friends Lists", function()
    notify("Friends lists reset", Color3.fromRGB(218, 38, 38))
end)
adjustWindowSizeToContent(friendsPanel, friendsContent)

-- 5. Create Macros Panel
macrosPanel, macrosContent = createPanel("Chat Macros Manager", 230, 160)
addTextboxOption(macrosContent, "Configure Macro Text", "Say something...", function(txt)
    S.MacroText = txt
    saveConfig()
    notify("Macro text configured!", Color3.fromRGB(50, 195, 75))
end)
addKeybindOption(macrosContent, "Trigger Macro Key", S.MacroKey or Enum.KeyCode.H, function(k)
    S.MacroKey = k
    saveConfig()
    notify("Macro trigger set to: " .. k.Name, Color3.fromRGB(50, 195, 75))
end)
adjustWindowSizeToContent(macrosPanel, macrosContent)

-- 6. Create Profiles Panel
profilesPanel, profilesContent = createPanel("Configurations Profiles", 230, 180)
addButtonOption(profilesContent, "Apply legit closet profile", function()
    Win:ResetAllToggles()
    S.WalkSpeed = 22
    S.JumpPower = 55
...
addButtonOption(profilesContent, "Apply blatant flight profile", function()
    Win:ResetAllToggles()
    S.Fly = true
    S.NoClip = true
    S.InfJump = true
...
addButtonOption(profilesContent, "Apply rage combat profile", function()
    Win:ResetAllToggles()
    S.Fly = true
    S.NoClip = true
    S.KillAura = true
...
adjustWindowSizeToContent(profilesPanel, profilesContent)

-- Tab buttons creation
local tabs = {"Modules", "Config", "GUI", "HUD", "Friends", "Macros", "Profiles"}
...
table.insert(S.Connections, UserInputService.InputBegan:Connect(function(inp, gpe)
    if inp.KeyCode == (S.UIToggleKey or Enum.KeyCode.RightControl) then
        toggleUIVisibility()
        return
    end
